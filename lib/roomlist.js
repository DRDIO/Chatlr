...
var RoomList = {
    ...
    isRoom: function(name, config)
    {   
        var room = this.list[name] || null;
        
        if (!room && config) {
            room = new Room(name, config.featured || false, config.topic || '');
    ...
    getList: function(isClient, isVisible, includeRoom)
    {
        ...
        if (isClient) {
            ...
            console.log('TIMER: Room Get List: ' + time + ' to ' + (new Date).getTime());
